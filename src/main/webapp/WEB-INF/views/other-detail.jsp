		        ...
		        <div id="Businessinfo_div" class="orgDetailModuleBusinessinfo">
				             ...
				             <table style="width:100%" id="orgDetail">
									<tr>
							            ...
							            <td>${empty proOtherInfo.pname ? '--':proOtherInfo.pname}</td>
									</tr>
									<tr>
							            <td align="right">产品类型：</td>
							            <td>${empty proOtherInfo.ptype ? '--':proOtherInfo.ptype}</td>
									</tr>
									<tr>
							            <td align="right">产品描述：</td>
							            <td>${empty proOtherInfo.pDesc ? '--':proOtherInfo.pDesc}</td>
									</tr>
									<tr>
							            <td align="right">产品评价：</td>
							            <td>${empty proOtherInfo.tag ? '--':proOtherInfo.tag}</td>
									</tr>
									<tr>
							            <td align="right">出版时间：</td>
							            <td>${empty proOtherInfo.ptime ? '--':proOtherInfo.ptime}</td>
									</tr>
									<tr>
							            <td align="right">团队规模：</td>
							            <td>${empty proOtherInfo.scale ? '--':proOtherInfo.scale}</td>
									</tr>
									<tr>
							            <td align="right">产品需求：</td>
							            <td>${empty proOtherInfo.pdemand ? '--':proOtherInfo.pdemand}</td>
									</tr>
									<tr>
							            <td align="right">图片链接：</td>
							            <td>${empty proOtherInfo.pictureUrl ? '--':proOtherInfo.pictureUrl}</td>
									</tr>
									<tr>
							            <td align="right">产品官网：</td>
							            <td>${empty proOtherInfo.web ? '--':proOtherInfo.web}</td>
									</tr>
									<tr>
							            <td align="right">信息来源：</td>
							            <td>${empty proOtherInfo.source ? '--':proOtherInfo.source}</td>
									</tr>
								<c:if test="${empty proOtherInfo }">
									<tr>
